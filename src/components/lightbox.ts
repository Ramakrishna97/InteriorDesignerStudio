import type { ProjectImage } from '../utils/types';

export function createLightbox(images: ProjectImage[]): HTMLElement {
  ...
  const currentSpan = lightbox.querySelector('.lightbox__current') as HTMLElement;
  const thumbnails = lightbox.querySelector('.lightbox__thumbnails') as HTMLElement;

  // Create thumbnails
  images.forEach((image, index) => {
    const thumb = document.createElement('img');
    thumb.className = 'lightbox__thumbnail';
    if (index === 0) thumb.classList.add('lightbox__thumbnail--active');
    thumb.src = image.url;
  ...
  // Public API
  (lightbox as any).show = (): void => {
    lightbox.classList.remove('lightbox--hidden');
    lightbox.setAttribute('aria-hidden', 'false');
  };

  (lightbox as any).hide = close;
